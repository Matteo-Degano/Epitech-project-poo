...
const fetchWorkingTimes = async () => {
  try {
    const response = await fetchData(
      "GET",
      `/workingtime/${authStore.user.id}?start=${lastWeek.toISOString()}&end=${currentDate.toISOString()}`
    )
...
const fetchChart = async () => {
  try {
    const response = await fetchData(
      "GET",
      `/chartmanager/${authStore.user.id}?start=${lastWeek.toISOString()}&end=${currentDate.toISOString()}`
    )
...
<template>
  ...
  <div v-else class="flex flex-col justify-around h-full gap-10">
    ...
    <div class="w-full h-96">
      <HoursWorkedChart v-if="dayTime" :data="dayTime" />
    </div>
  </div>
</template>
