...
<template>
    <div class="bg-white p-6 rounded-lg shadow-sm max-w-md mx-auto">
      ...
      <input 
        ...
        placeholder="User ID"
      >
      <span class="text-xs text-gray-500">Entrer l'id de l'utilisateur pour obtenir les informations (système temporaire tant que y'a pas d'auth mdrr)</span>
  
      <div v-if="mode === 'view'" class="mt-5">
          <h2 class="font-medium text-gray-700">Username: <span class="text-gray-900">{{ user.username }}</span></h2>
          <h2 class="font-medium text-gray-700">Email: <span class="text-gray-900">{{ user.email }}</span></h2>
      </div>
  
      <div v-else class="mt-5">
          <h2 class="font-medium text-gray-800 mb-3">{{ mode === 'edit' ? 'Edit User' : 'Create New User' }}</h2>
          <input 
            type="text" 
      ...
      <div class="mt-5 flex gap-2">
          <button class="flex-1 bg-gray-100 text-gray-800 py-2 rounded-lg hover:bg-gray-200 transition" @click="getUser">Get User</button>
          <button class="flex-1 bg-gray-100 text-gray-800 py-2 rounded-lg hover:bg-gray-200 transition" @click="() => toggleMode('edit')">Edit</button>
          <button class="flex-1 bg-gray-100 text-red-600 py-2 rounded-lg hover:bg-gray-200 transition" @click="deleteUser">Delete</button>
          <button class="flex-1 bg-gray-100 text-gray-800 py-2 rounded-lg hover:bg-gray-200 transition" @click="() => toggleMode('create')">Create</button>
      </div>
    </div>
  </template>
